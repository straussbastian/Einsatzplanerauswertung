...
6. **LLM als Onboarding-Kalibrator statt als Tages-Kalibrator.** Die plausibelste Produktidee, die aus dem Negativbefund folgt, verschiebt den LLM-Einsatz komplett weg vom laufenden Betrieb: **einmal bei der Inbetriebnahme** analysiert das LLM das Betriebsprofil eines neuen Kunden — Radius, Techniker-Zahl, Schicht-Modell, typische Auftragsdichte, SLA-Regeln, Qualifikationsmix, Saisonalität — und wählt dafür einen initialen Gewichts-Vektor (base_penalty, notfall_bonus, travel_weight_pct, sla_*_bonus etc.). Danach läuft der Solver statisch, ohne LLM im Loop.

   Das ist **nicht** der Hybrid dieser Arbeit und wird durch die vier Haupt-Tests nicht widerlegt: wir haben getestet, dass tägliche Umparametrisierung im laufenden Betrieb nichts bringt, weil das Gewichts-Plateau flach ist (§7.0). Wir haben **nicht** getestet, ob man für einen noch nie gesehenen Betrieb den Einstieg ins Plateau automatisiert finden kann. Das ist ein konzeptuell anderes Problem: Kaltstart, nicht laufender Betrieb. Für einen Mittelstandsbetrieb (§6), der weder die Penalty-Mechanik versteht noch eine OR-Abteilung hat, ist genau das die Entscheidung, die er nicht treffen kann.

   Die strengere Produkt-Hypothese: *LLM-kalibriert vs. einheitlicher Default bei einer Bandbreite unterschiedlicher Betriebsprofile* (nicht bei einer festen Konstellation wie in §5). Der Benchmark dafür existiert noch nicht — er müsste über synthetische Profil-Varianten laufen (10 Techs 30 km vs. 40 Techs 80 km Multi-Depot vs. 5 Techs SLA-lastig etc.) und prüfen, ob das LLM-Onboarding in genug Profilen einen messbaren Vorsprung gegen den einheitlichen `normal`-Default aufbaut, um die einmaligen Onboarding-Kosten zu rechtfertigen. Kostenstruktur passt: einmalige LLM-Kosten (<$0.20) gegen die Betriebs-Lebensdauer des Solvers.
